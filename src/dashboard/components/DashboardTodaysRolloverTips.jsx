...
const DashboardTodaysRolloverTips = () => {
  ...
  if (loading) {
    return <p className="text-white">Loading today's rollover tips...</p>;
  }
      ...
      {tipsByPlan.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {tipsByPlan.map(({ plan, tip }) => (
            <div
              key={tip._id}
              className="bg-[#1c223f] rounded-xl p-4 border border-yellow-500"
            >
              ...
              {tip.note && (
                <p className="mt-2 italic text-sm text-white/80">Note: {tip.note}</p>
              )}
